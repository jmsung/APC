...
def processive_step(X, Z):
    X_processive = np.zeros((len(X), 1))
    Z_processive = np.zeros((len(X), 1))
    ...
    for i in range(len(Z)-1):
        if Z[i] > Z[i+1]:
            Z_processive[i+1:] = Z_processive[i+1:] + 2 + 0.5*np.random.randn()
        else:
            Z_processive[i+1:] = Z_processive[i+1:]            
            
    Noise = X - Z
    
    X_processive = Z_processive + Noise
        
        
    return X_processive, Z_processive
...
class Data:
    ...
    def generate(self):       
        ...
        self.X_processive = [None] * self.n_sample
        self.Z_processive = [None] * self.n_sample        
        ...
        for i in range(self.n_sample):
            ...
            self.X_processive[i], self.Z_processive[i] = processive_step(self.X[i], self.Z_true[i])
...
#            percent_error = sum(abs(self.Z_true[i] - self.Z_predict[i]))/self.n_frame*100
            ...
            sp.plot(self.Z_processive[i], "r", ms=1, lw=2, alpha=1)              
            sp.plot(self.X_processive[i], "k", ms=2, lw=2, alpha=0.5)                           
#            sp.plot(X_true, "b", label="states", lw=2, alpha=1)
#            sp.plot(X_predict, "r", label="predictions", lw=2, alpha=1)
            sp.set_ylabel('Position')
            sp.set_xlabel('Frame')
            sp.set_title("SNR = %.1f, HMM" % (self.SNR)) #show both signal and noise
